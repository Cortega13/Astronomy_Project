...
class DatasetConfig:
    ...
    scalers_path = os.path.join(working_path, "data/scalers.plk")
...
class PredefinedTensors:
    ab_min, ab_max = DatasetConfig.scalers["total_species"]
    ab_min = torch.tensor(ab_min, dtype=torch.float32).to(device)
    ab_max = torch.tensor(ab_max, dtype=torch.float32).to(device)
